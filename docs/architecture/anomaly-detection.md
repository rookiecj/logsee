...
| 소비자 | 화면 하나 | TUI + JSON + CLI + webhook |
...
- **TUI 비의존 코어**: 분석 파이프라인이 Bubble Tea 없이도 동작한다(headless JSONL).
...
```
┌─────────────────────────────────────────────────────────┐
│ Outbound adapters   TUI │ JSON │ CLI │ webhook          │
├─────────────────────────────────────────────────────────┤
...
TUI bus, JSONL writer, webhook notifier 모두 동일 포트로 꽂힌다.
...
- **영속화 드라이버**: v1은 순수 인메모리. `Store`는 세션 종료 시 사라짐. JSONL/bbolt 드라이버 결정은 실사용 측정 후 판단. [pending]
