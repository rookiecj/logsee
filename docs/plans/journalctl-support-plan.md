...
- **`journalctl` 프로세스를 logsee가 직접 스폰하지 않는다.** 사용자는 파이프(`journalctl -o short-iso-precise | logsee …`) 또는 저장된 덤프 파일(`logsee journal.log`) 로 입력을 제공. 이유: 크로스플랫폼 / 권한 / 프로세스 수명 관리 부담 회피, 기존 `source.FileSource` / `source.ReaderSource` 재사용.
- systemd D-Bus / libjournal 바인딩.
- 바이너리 `.journal` 파일 직접 파싱. 사용자가 `journalctl` 로 풀어서 제공한다고 가정.
...
3-layer 설계 그대로. 새 슬롯만 채운다. **새 Source 어댑터는 추가하지 않음** — 기존 `FileSource`/`ReaderSource`가 그대로 journalctl 덤프·파이프를 읽는다.

```
Source     : 기존 source.FileSource / source.ReaderSource (변경 없음)
                │
...
어떤 기존 파일도 **의미를 바꾸지 않는다**. 규칙 테이블은 format 가드(FormatAndroid / FormatSystemdJournal)로 분리해 충돌을 막는다.

사용 예:

```bash
# 파이프
journalctl -u nginx.service -f -o short-iso-precise | logsee --log-type=journal

# 저장 덤프 파일
journalctl -b -o short-iso-precise > boot.journal
logsee --log-type=journal boot.journal
logsee --export-anomalies --log-type=journal boot.journal > boot.anomalies.jsonl
```
...
당초 subprocess 기반 `JournalctlSource` 를 두려 했으나, 파이프/파일 입력으로 전부 커버되어 제거. 사용자는 `journalctl -o short-iso-precise | logsee …` 또는 덤프 파일을 인자로 전달한다.
...
| v1.11.0 | J3 | Tier A systemd/kernel 규칙 (J2는 제거됨) |
...
- **바이너리 `.journal` 지원 없음** — 사용자가 `journalctl` 로 텍스트/JSON 출력해서 제공해야 한다는 점을 문서에 명시. logsee 자체는 journalctl 프로세스를 스폰하지 않음.
