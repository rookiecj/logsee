	...
	return []helpSection{
		...
		},
		{
			...
			rows: [][2]string{
				...
				{"anomaly:any", "분류기가 탐지한 이상(ANR·FATAL·tombstone 등) 전체"},
				{"anomaly:anr", "특정 종류만: anr·fatal_java·native_crash_header·watchdog·lmk_kill·oom 등"},
				{"anomaly:any -anomaly:oom", "oom 제외한 모든 이상"},
			},
		},
		{
			title: "필터 입력 키",
	...
	return []helpSection{
		...
		},
		{
			...
			rows: [][2]string{
				...
				{"level:E  level:W", "예약 태그 level (E/W/I/D/V/F)"},
				{"anomaly:anr", "예약 태그 anomaly (any 또는 kind). A키는 any 토글"},
